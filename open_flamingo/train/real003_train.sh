...
torchrun --nnodes=1 --nproc_per_node=${WORLD_SIZE}  real2code_train.py \
  ...
  --dataset_resampled \
  --batch_size_mujoco 1 \
  --train_num_samples_mujoco 424 --workers 2 --num_epochs 100 \
  --warmup_steps  187 \
  --fsdp --fsdp_use_orig_params \
  --report_to_wandb \
  --cross_attn_every_n_layers 20 --gradient_accumulation_steps 4 \
  --run_name /local/real/mandi/flg_data/test-Codellama-20Layer --learning_rate 3e-4 --report_to_wandb
